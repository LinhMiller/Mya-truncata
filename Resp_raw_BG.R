...
bxp <- ggboxplot (DT, x= "time", y = "Oxygen", colour= "Treatment", palette= "jco",
                  outlier.shape = NA
)
...
#bxp<-bxp + labs(title = "Oxygen Consumption Before and After Ramping",
                #x = "Time",y = "mgO2/hour/kg",colour = "Treatment") +
  #theme(text = element_text(size=14))

#ggsave(plot=bxp, filename= "output/Result_graph/before_after_plot_newBG.png")
#####with outlier shape
...
all<-annotate_figure(fig,
 ...
 ggsave(plot=all, file="./output/Result_graph/trt_pannels_newBG.jpeg", height= 10, width = 6.8, units="in")
all
#### BG plot
dt <-read_excel("./Mydata/Rimouski_Sum_all_raw_newBG.xlsx")
